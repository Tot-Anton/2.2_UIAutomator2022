...
//    }
    ...
    @Test
    fun testForSettingAnEmptyString() {
        val textToSet = " "
        val textToSpeechObject2 = "Hello UiAutomator!"
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)
        ...
        val result = device.findObject(By.res(packageName, "textToBeChanged")).text
        assertEquals(result, textToSpeechObject2)
    }

    @Test
    fun testForOpeningTextInNewActivity() {
        val textToSet = "AndroidTest"
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)
